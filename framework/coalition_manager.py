...
class CoalitionManager:
    ...
    def plot_coalition(
        ...
        for uav_id in coalition:
            ...
            for i in range(len(bezier_points) - 1):
                ...
                line = plt.Line2D(
                    [bezier_points[i][0], bezier_points[i + 1][0]],
            ...
            arrow_head = Polygon(
                [
                    ...
                    end_point
                    - arrow_size
                    * np.array([np.cos(arrow_angle + np.pi / 6), np.sin(arrow_angle + np.pi / 6)]),
                    end_point
                    - 0.5 * arrow_size * np.array([np.cos(arrow_angle), np.sin(arrow_angle)]),
                    end_point
                    - arrow_size
                    * np.array([np.cos(arrow_angle - np.pi / 6), np.sin(arrow_angle - np.pi / 6)]),
                ],
    ...
    def plot_map(
        ...
        task_list = [
            task for task in task_manager.get_all() if task.id != task_manager.free_uav_task_id
        ]
        ...
        stats_box = plt.text(
            ...
            va="bottom",
            bbox=dict(
                boxstyle="round,pad=0.6", facecolor="white", edgecolor="lightgray", alpha=0.8
            ),
            zorder=5,
        )
        ...
        assigned_tasks = len(
            [t for t, c in self.task2coalition.items() if t != self.free_uav_task_id and c]
        )
        ...
        plt.figtext(
            0.01, 0.01, f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}", fontsize=8, color="gray"
        )
