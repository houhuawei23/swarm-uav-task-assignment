...
class BruteForceSearchSolver(MRTASolver):
    ...
    def run_allocate(self) -> Dict[int, List[int]]:
        ...
        initial_state = SearchState(
            assignment={task_id: [] for task_id in self.task_manager.get_ids()},
        )
        ...
        with tqdm(total=len(self.uav_manager), desc="Searching assignments") as pbar:
            try:
                best_assignment, best_score = self._search(
                    initial_state, best_assignment, best_score, theoretical_max, pbar
                )
            except TimeoutError:
        ...
        print(f"best_assignment: {best_assignment}")
        if best_assignment:
            self.coalition_manager.update_from_assignment(best_assignment, self.uav_manager)
        print(f"best_assignment: {best_assignment}")
    ...
    def _search(
        ...
        参数说明：
        - state: 包含当前分配状态、未分配UAV集合等信息的搜索状态
        - best_assignment: 当前找到的最佳分配方案
        - best_score: 当前最佳分数
        - theoretical_max: 理论最大可能分数，用于提前终止
        - pbar: 进度条对象，用于显示搜索进度
        ...
        print(
            f"state: {state}, best_assignment: {best_assignment}, best_score: {best_score}, theoretical_max: {theoretical_max}"
        )
        # Check time limit
    ...
    def _evaluate_assignment(self, assignment: Dict[int, List[int]]) -> float:
        ...
        for task_id, uav_ids in assignment.items():
            ...
            resource_satisfaction = (
                np.sum(obtained_resources >= task.required_resources)
                / self.hyper_params.resources_num
            )

            # Calculate resource efficiency
            if np.sum(obtained_resources) > 0:
                resource_efficiency = 1 - np.sum(
    ...
    def _calculate_uav_benefit(
        self, uav: UAV, task: Task, coalition: List[int], obtained_resources: np.ndarray
    ) -> float:
        ...
        # Combine factors
        weights = {"resource": 3.0, "path": 2.0, "threat": 1.0, "time": 4.0}
    ...
    def _calculate_max_possible_benefit(self, uav: UAV, task: Task) -> float:
        ...
        # Combine factors with weights
        weights = {"resource": 3.0, "path": 2.0, "time": 4.0}
