...
from framework.base import HyperParams, LogLevel, Point
...
logger = logging.getLogger("DistributedMultiThreadSolver")
...
@dataclass
class Message:
    """Message class for communication between UAV threads."""
    uav_id: int
    changed: bool
...
class AutoUAVThread(UAV):
    """Thread-safe version of AutoUAV that runs in its own thread."""
    
    def __init__(
        self,
        id: int,
        position: List[float] | np.ndarray | Point,
    ...
    def start(self):
        """Start the UAV's thread."""
        self.thread = threading.Thread(target=self._run, daemon=True)
        self.thread.start()

    def stop(self):
        """Stop the UAV's thread."""
        self.stop_event.set()
        if self.thread:
            self.thread.join()

    def _run(self):
        """Main thread loop for the UAV."""
        while not self.stop_event.is_set():
            try:
                ...
                logger.error(f"Error in UAV {self.id} thread: {str(e)}")
...
class DistributedSolver_MultiThreading(MRTASolver):
    """Multithreaded version of the distributed solver."""
    ...
    def __init__(
        ...
        self.uav_threads: Dict[int, AutoUAVThread] = {}
        self.message_queues: Dict[int, queue.Queue] = {}
        self.stop_event = threading.Event()

    @staticmethod
    def uav_type() -> Type:
        return AutoUAVThread

    @classmethod
    def type_name(cls):
        return "DistributedMultiThreading"

    def init_allocate(self, components: List[List[int]], debug=False):
        """Initialize UAV threads and their allocations."""
        ...
        for component in components:
            ...
            for uav_id in component:
                ...
                self.uav_threads[uav_id] = uav
                self.message_queues[uav_id] = queue.Queue()

    def run_allocate(self):
        """Run the multithreaded allocation algorithm."""
        ...
        for uav in self.uav_threads.values():
            uav.start()
        ...
        try:
            ...
            while True:
                ...
                for component in components:
                    ...
                    leader_uav = self.uav_threads[leader_uav_id]
